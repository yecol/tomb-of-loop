...
static NSString *const LCCKAPPID = @"6P22nVYj4LxDWIbPH18Nl1d0-MdYXbMMI";
static NSString *const LCCKAPPKEY = @"CAkV248oXda09OUM9BHssly3";

@implementation LPChat (Setting)

- (void)lpchat_setting {
    [self lpchat_setupAppInfo];
    //设置用户体系
    [self lcck_setFetchProfiles];
    //设置签名机制
    //[self lcck_setGenerateSignature];
    //设置聊天列表
    [self lpchat_setupConversationsList];
    //设置聊天
    [self lpchat_setupConversation];
    // 其他各种设置
    [self lpchat_setupOther];
}

- (void)lpchat_setupConversationsList {
    //设置最近联系人列表cell的操作
    [self lpchat_setupConversationsCellOperation];
    //自定义Cell菜单
    [self lpchat_setupConversationEditActionForConversationList];
}

- (void)lpchat_setupConversation {
    //设置打开会话的操作
    [self lpchat_setupOpenConversation];
    [self lpchat_setupConversationInvalidedHandler];
    [self lpchat_setupLoadLatestMessages];
    //点击图片，放大查看的设置。不设置则使用默认方式
    //[self lcck_setupPreviewImageMessage];
    [self lpchat_setupLongPressMessage];
}

- (void)lpchat_setupOther {
    //TabBar样式，自动设置。如果不是TabBar样式，再实现该方法
    //[self lcck_setupBadge];
    [self lpchat_setupForceReconect];
    [self lpchat_setupHud];
    [self lpchat_setupOpenProfile];
    //开启圆角
    //[self lcck_setupAvatarImageCornerRadius];
    //筛选消息
    //[self lcck_setupFilterMessage];
    [self lpchat_setupNotification];
    [self lpchat_setupPreviewLocationMessage];
}

#pragma mark - leanCloud的app信息设置
- (void)lpchat_setupAppInfo {
...
#warning 注意：以下方法循环模拟了通过 userIds 同步查询 users 信息的过程，这里需要替换为 App 的 API 同步查询
        ...
        [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
            if(error != nil){
                NSLog(@"yecolfetch error fetch");
//                users.addO
            }
            else{
                NSLog(@"yecolfetch get users = %@", objects);
                for(NSObject *user in objects){
                    AVUser *avuser = (AVUser*)user;
                    NSDictionary *userDict = (NSDictionary*)user;
                    NSString *username = [NSString stringWithFormat:@"%@ %@",[userDict objectForKey:@"firstName"],[userDict objectForKey:@"lastName"]];
                    AVFile *avatarFile = (AVFile*)[userDict objectForKey:@"avatar"];
                    NSURL *url = [NSURL URLWithString:avatarFile.url];
                    NSLog(@"username= %@, avatar=%@ ", username, url);
                    
                    LCCKUser *luser = [LCCKUser userWithUserId:avuser.objectId name:username avatarURL:url clientId:avuser.objectId];
                    [users addObject:luser];
                }
            }
...
//                AVUser *user_ = [LCCKUser userWithUserId:user[LCCKProfileKeyPeerId]
...
- (void)lpchat_setupConversationEditActionForConversationList {
    // 自定义Cell菜单
    [[LCChatKit sharedInstance] setConversationEditActionBlock:^NSArray *(
        ...
        return [self lpchat_conversationEditActionAtIndexPath:indexPath
 ...
 */
- (void)lpchat_setupConversationsCellOperation {
    //选中某个对话后的回调,设置事件响应函数
    [[LCChatKit sharedInstance] setDidSelectConversationsListCellBlock:^(
        ...
        [[self class] LPOpenConversationViewControllerWithConversaionId:conversation.conversationId
 ...
 *  打开一个会话的操作
 */
- (void)lpchat_setupOpenConversation {
 ...
 *  设置会话出错的回调处理
 */
- (void)lpchat_setupConversationInvalidedHandler {
 ...
 *  加载最近聊天记录的回调
 */
- (void)lpchat_setupLoadLatestMessages {
 ...
 */
- (void)lpchat_setupPreviewImageMessage {
 ...
 *  设置会话界面的长按操作
 */
- (void)lpchat_setupLongPressMessage {
    [[LCChatKit sharedInstance] setLongPressMessageBlock:^NSArray<UIMenuItem *> *(
        ...
        LCCKMenuItem *transpondItem = [[LCCKMenuItem alloc]
                                       ...
                                       block:^{
                                           [self lpchat_transpondMessage:message
...
- (void)lpchat_setupBadge {
    ...
    [[LCChatKit sharedInstance] setMarkBadgeWithTotalUnreadCountBlock:^(
        ...
        [self lpchat_markBadgeWithTotalUnreadCount:totalUnreadCount controller:controller];
    }];
}
 ...
 *  强制重连
 */
- (void)lpchat_setupForceReconect {
...
//                                                            [[self class] lcck_hideHUDForView:viewController.view];
        ...
        // 用户拒绝了重连请求
        // - 退回登录页面
        [[self class] lpchat_clearLocalClientInfo];
        // - 显示返回信息
        NSInteger code = 0;
...
- (void)lpchat_setupOpenProfile {
    [[LCChatKit sharedInstance] setOpenProfileBlock:^(NSString *userId, id<LCCKUserDelegate> user,
        ...
        [self lpchat_openProfileForUser:user userId:userId parentController:parentController];
    }];
}
 ...
 *   头像开启圆角设置
 */
- (void)lpchat_setupAvatarImageCornerRadius {
 ...
 *  筛选消息的设置
 */
- (void)lpchat_setupFilterMessage {
...
- (void)lpchat_setupNotification {
    [[LCChatKit sharedInstance] setShowNotificationBlock:^(UIViewController *viewController, NSString *title,
        ...
        [self lpchat_showNotificationWithTitle:title subtitle:subtitle type:type];
    }];
}
 ...
 *  设置预览定位样式
 */
- (void)lpchat_setupPreviewLocationMessage {
    [[LCChatKit sharedInstance] setPreviewLocationMessageBlock:^(CLLocation *location, NSString *geolocations,
                                                                 NSDictionary *userInfo) {
        [self lpchat_preViewLocationMessageWithLocation:location geolocations:geolocations];
    }];
}

#pragma mark - private
- (void)lpchat_showNotificationWithTitle:(NSString *)title
                                     subtitle:(NSString *)subtitle
                                         type:(LCCKMessageNotificationType)type {
    [LCCKUtil showNotificationWithTitle:title subtitle:subtitle type:type];
}

- (void)lpchat_openProfileForUser:(id<LCCKUserDelegate>)user
...
//        [[self class] lpchat_pushToViewController:conversationViewController_];
//        return;
//    }
//    [LCCKUtil showNotificationWithTitle:title
//                               subtitle:subtitle
...
- (void)lpchat_preViewLocationMessageWithLocation:(CLLocation *)location
 ...
 */
- (void)lpchat_markBadgeWithTotalUnreadCount:(NSInteger)totalUnreadCount
 ...
 */
- (NSArray *)lpchat_conversationEditActionAtIndexPath:(NSIndexPath *)indexPath
                                              conversation:(AVIMConversation *)conversation
                                                controller:(LCCKConversationListViewController *)controller {
    // 如果需要自定义其他会话的菜单，在此编辑
    return [self lpchat_rightButtonsAtIndexPath:indexPath conversation:conversation controller:controller];
}

- (NSArray *)lpchat_rightButtonsAtIndexPath:(NSIndexPath *)indexPath
                             conversation:(AVIMConversation *)conversation
                               controller:(LCCKConversationListViewController *)controller {
    NSString *title = nil;
    UITableViewRowActionHandler handler = nil;
    [self lpchat_markReadStatusAtIndexPath:indexPath
                                   title:&title
                                  handle:&handler
                            conversation:conversation
                              controller:controller];
    ...
    UITableViewRowAction *actionItemChangeGroupAvatar = [UITableViewRowAction
                                                         ...
                                                         title:@"改群头像"
                                                         handler:^(UITableViewRowAction *action, NSIndexPath *indexPath) {
                                                             [[self class] lpchat_changeGroupAvatarURLsForConversationId:conversation.conversationId
                                                                                                                 shouldInsert:NO];
                                                         }];
...
- (void)lpchat_markReadStatusAtIndexPath:(NSIndexPath *)indexPath
...
- (void)lpchat_transpondMessage:(LCCKMessage *)message
